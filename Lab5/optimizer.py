...
# -*- coding: utf-8 -*-
"""
Created on Wed Nov 18 13:30:29 2020
...
def force(pkCurr, map, pkGoal):
    ...
    rep, dist =  force_repulsive(pkCurr, map)
...
def joint_efforts_individual(joint, qCurr, map, qGoal, points_on_link): #
    ...
    for i, Jv in enumerate(Jvk):
      Fk, dist = force(pkCurr[i], map, pkGoal[i])
...
def joint_efforts_combination(qCurr, map, qGoal):
    ...
    for k in range(2,6):
        t, dist = joint_efforts_individual(k, qCurr, map, qGoal, points_on_link)
        if(dist==0):
            break
        tau += t
    return tau, dist
...
def potentialFieldStep(qCurr, map, qGoal, alpha=1e-3):
    ...
    """
    epsilon = 1e-2      #TODO isDone condition
    tau, dist = joint_efforts_combination(qCurr, map, qGoal) 
